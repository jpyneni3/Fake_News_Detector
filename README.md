...
Developed by [Jas Pyneni](https://github.com/jpyneni3) and [Shukan Shah](https://github.com/shukieshah)

Fake News Detector is a project that aims to distinguish between human-written fake news and real news. To accomplish this, we trained state-of-the-art NLP models to classify different news articles and subsequently improved their performance by using various data augmentation techniques. We performed comparative analysis among the different methods and presented our results in this [paper](Paper.pdf). Finally, we deployed our best model and released a user-friendly [web app](https://shukieshah.github.io/VerifAI) to the wider community that takes in text and classifies whether that piece of news is fake or real.

This repository contains the large corpus of fake and real news data. It also contains the notebooks used for scraping, cleaning, and augmenting data as well as training the classification models. Due to GitHub file size limits, we have not uploaded the final trained models. If you would like to reproduce the results and experiments outlined in the paper, we highly recommend running the notebooks on [Google Colaboratory](https://colab.research.google.com/notebooks/intro.ipynb#recent=true).
